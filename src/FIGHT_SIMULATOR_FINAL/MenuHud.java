/**
 * Cette classe sert d'habillage pour le menu, elle comporte tous les boutons et actions associée dans la composition
 * des équipes
 */
package projet_fin;
...
public class MenuHud implements ComponentListener{
	...
	/**
	 * On va initialiser tous les boutons et définir les zones cliquables
	 * @param container fenetre dans laquelle on se trouve
	 * @param game couche de l'application
	 */
	public void init(GameContainer container, StateBasedGame game) throws SlickException {
		...
		// on definit les zones cliquables relatifs aux piquiers (ajouter et supprimer pour les deux équipes)
		...
		// on definit les zones cliquables relatifs aux chevaliers (ajouter et supprimer pour les deux équipes)
		...
		// on definit les zones cliquables relatifs aux archers (ajouter et supprimer pour les deux équipes)
	 ...
	 * @param container fenetre sur laquelle on interagit
	 * @param g graphic sur lequel on dessine
	 */
	public void render(GameContainer container, Graphics g) { 
	  g.setColor(Color.black); // on veut écrire en noir
	  ...
	  // le label et le nombre de piquier par equipe
	  ...
	  // le label et le nombre de chevalier par equipe
	  ...
	  // le label et le nombre d'archer par equipe
	...
	public void afficher_bouton(GameContainer container, Graphics g) {
		...
		change_map.render(container, g); // nouton changer de carte
		g.drawString("Changer Carte", change_map.getX() + X_PADDING , change_map.getY() + Y_PADDING);
		
		// les boutons relatifs aux piquiers
		...
		// les boutons relatifs aux chevaliers
		...
		// les boutons relatifs aux archers
		...
		// les boutons pour les bonus ainsi que le label "bonus"
	...
	/**
	 * Fonction qui se lance lorque l'utilisateur interagit avec une zone cliquable
	 * On adopte un comportement différent en fonction de la zone
	 */
	public void componentActivated(AbstractComponent source) {
		...
		if (source == chevalier.getAdd_unite1()) {
			chevalier.setNb_unite1(chevalier.getNb_unite1() + 1); // ajout d'un chevalier equipe 1
		} else if (source == chevalier.getAdd_unite2()) {
			chevalier.setNb_unite2(chevalier.getNb_unite2() + 1); // ajout d'un chevalier equipe 2
		}else if(source == piquier.getAdd_unite1()) {
			piquier.setNb_unite1(piquier.getNb_unite1() + 1);// ajout d'un piquier equipe 1
		}else if(source == piquier.getAdd_unite2()) {
			piquier.setNb_unite2(piquier.getNb_unite2() + 1); // ajout d'un piquier equipe 2
		}else if(source == archer.getAdd_unite1()) {
			archer.setNb_unite1(archer.getNb_unite1() + 1); // ajout d'un archer equipe 1
		}else if(source == archer.getAdd_unite2()) {
			archer.setNb_unite2(archer.getNb_unite2() + 1); // ajout d'un archer equipe 2
		}else if(source == add_bonus) {
			nb_bonus++; // ajout d'un bonus
		}else if(source == chevalier.getDelete_unite1()) { // suppresion d'un chevalier equipe 1
			if(chevalier.getNb_unite1() > 0) {
				chevalier.setNb_unite1(chevalier.getNb_unite1() - 1);
			}
		}else if(source == chevalier.getDelete_unite2()) { // suppresion d'un chevalier equipe 2
			if(chevalier.getNb_unite2() > 0) {
				chevalier.setNb_unite2(chevalier.getNb_unite2() - 1);
			}
		}else if(source == archer.getDelete_unite1()) { // suppresion d'un archer equipe 1
			if(archer.getNb_unite1() > 0) {
				archer.setNb_unite1(archer.getNb_unite1() - 1);
			}
		}else if(source == archer.getDelete_unite2()) { // suppresion d'un archer equipe 2
			if(archer.getNb_unite2() > 0) {
				archer.setNb_unite2(archer.getNb_unite2() - 1);
			}
		}else if(source == piquier.getDelete_unite1()) { // suppresion d'un piquier equipe 1
			if(piquier.getNb_unite1() > 0) {
				piquier.setNb_unite1(piquier.getNb_unite1() - 1);
			}
		}else if(source == piquier.getDelete_unite2()) { // suppresion d'un piquier equipe 2
			if(piquier.getNb_unite2() > 0) {
				piquier.setNb_unite2(piquier.getNb_unite2() - 1);
			}
		}else if(source == delete_bonus) { // suppresion d'un bonus
			if(nb_bonus > 0) {
				nb_bonus--;
			}
		}else if(source == change_map) { // changement de map
			num_map = (num_map+ 1)%3;
			...
			// puis on change de couche
			  ...
			  ((MapGameState2) game.getState(MapGameState2.ID)).getE1().setNb_chevalier(chevalier.getNb_unite1());
			  ((MapGameState2) game.getState(MapGameState2.ID)).getE2().setNb_chevalier(chevalier.getNb_unite2());
			  ((MapGameState2) game.getState(MapGameState2.ID)).getE1().setNb_piquier(piquier.getNb_unite1());
			  ((MapGameState2) game.getState(MapGameState2.ID)).getE2().setNb_piquier(piquier.getNb_unite2());
			  ((MapGameState2) game.getState(MapGameState2.ID)).getE1().setNb_archer(archer.getNb_unite1());
			  ((MapGameState2) game.getState(MapGameState2.ID)).getE2().setNb_archer(archer.getNb_unite2());
